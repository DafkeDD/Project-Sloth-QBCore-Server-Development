			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						if fromItemData.info.quality ~= toItemData.info.quality then
							TriggerClientEvent("QBCore:Notify",src ,"You can not stack item which are not with same quality",'error')
						else
